...
class RedisChatMessageHistory(BaseChatMessageHistory):
    def __init__(self, session_id: str, ttl: int = None):
        ...
        self.ttl = ttl if ttl is not None else agent_config["redis"]["ttl"]# 默认过期时间
        
        # 从配置读取Redis连接参数，支持环境变量覆盖（Docker用）
        redis_config = agent_config["redis"]
        redis_host = os.environ.get("REDIS_HOST", redis_config["host"])
        try:
            self.redis = redis.Redis(
                host=redis_host,# Redis主机地址
                port=redis_config["port"],# Redis端口号
                db=redis_config["db"],# Redis数据库索引
                decode_responses=False,
                socket_timeout=5# 默认超时时间
            )
            # 测试连接
            self.redis.ping()
